...
df1 = df.weight/(df.height*df.height*0.0001)
df['overweight'] = df1
...
def draw_cat_plot():
    ...
    df_cat = df_cat.groupby(['cardio','variable'], as_index = False)['value'].value_counts()
...
def draw_heat_map():
    ...
    ax = sns.heatmap(data=corr,mask=mask, annot=True, fmt='.1f', center=0,square=True,linewidths=.5)
